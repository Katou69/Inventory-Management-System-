...
export default function CapacityCard({ wh }: { wh: WarehouseDetail }) {
  ...
  return (
    <div className="bg-card rounded-xl border border-border shadow-sm p-6 flex-1 min-w-0 flex flex-col">
      ...
      <p className="text-xs text-muted-foreground mt-0.5">{wh.name} • Total {total.toLocaleString()} units</p>
      ...
      <div className="flex flex-1 items-center justify-center gap-8 mt-5 flex-wrap">
        <div className="relative shrink-0" style={{ width: size, height: size }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <p className="text-2xl font-bold text-foreground leading-none">{pct.toFixed(1)}%</p>
...
function LegendItem({ color, label, pct, units }: { color: string; label: string; pct: number; units: number }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <p className="text-sm font-medium text-foreground">{label} — {pct.toFixed(1)}%</p>
